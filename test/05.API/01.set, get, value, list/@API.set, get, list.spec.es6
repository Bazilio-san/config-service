...
describe('API: set(), get(), getEx(), list()', () => {
    ...
    describe('Check new values by get():', () => {
        ...
        it('Method getEx() should return the correct value', () => {
            const configName = 'config1';
            const newConfig = require('../../04.Params/07._updateAndSaveNamedConfig/new-config1.es6');
            instance._updateAndSaveNamedConfig(configName, newConfig);
            const content = instance.getEx('config1');
            const expected2 = require('./expected-ansver1.es6');
    ...
    describe('ERRORS: set', () => {
        ...
        it(`If the value is set incorrectly, the old value should remain`, () => {
            expect(instance.get('config1.div13.v_email', 'test'))
