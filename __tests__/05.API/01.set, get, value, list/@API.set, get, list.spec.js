...
describe('API: set(), get(), getEx(), list()', () => {
  ...
  test('After set(): check new value in a configuration object', () => {
  ...
  describe('Check new values by get():', () => {
        lib.each(expected, (configExpected, configName) => {
          test(`Named config: "${configName}"`, () => {
            const paramValue = instance.get(configName);
                expect(paramValue).toMatchObject(configExpected);
          });
        });
        [...newValues, ...unchangedValues].forEach(([paramPath, expectedValue, overwrittenValue]) => {
          test(`get(): path: "${paramPath}"`, () => {
            const paramValue = instance.get(paramPath);
                expect(paramValue).toMatchObject(overwrittenValue || expectedValue);
          });
        });
        test('Method getEx() should return the correct value', () => {
          ...
          const newConfig = require('../../04.Params/07._updateAndSaveNamedConfig/new-config1.js');
            ...
            expect(content).toMatchObject(expected2);
        });
  });
  ...
  describe('ERRORS: set', () => {
        [
          ...
          test(`ERROR: Cannot set a value for a 'section': '${path}' <- ${value}`, () => {
        ...
        [
          ...
          test(`ERROR: Invalid type of param: '${path}' <- ${value}`, () => {
                expect(niError('API', 'set', path, value))
        ...
        [
          ...
          test(`ERROR: Parameter could not be normalized: '${path}' <- ${typeof v === 'number' ? v : `'${v}'`}`, () => {
                expect(niError('API', 'set', path, v))
                    .to.match(/Validation error/);
          });
        });

        test('ERROR: The path must begin with a named configuration identifier', () => {
            expect(fnError(instance, 'set', '', 'text'))
                .to.match(/The path must begin with a named configuration identifier/);
        });

        test('ERROR: There is no such named configuration in the schema', () => {
        ...
        test(`If the value is set incorrectly, the old value should remain`, () => {
  ...
  describe('Check list():', () => {
    test('list should return list of configNames', () => {
            expect(instance.list()).toMatchObject(['config1', 'config-2', 'config 3']);
    });
  });
