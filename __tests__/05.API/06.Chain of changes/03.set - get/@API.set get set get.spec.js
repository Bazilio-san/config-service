      ...
      describe(`Change ${n}`, () => {
        ...
        test(`getSchema()`, () => {
          const val = instance.getSchema();
                expect(val).toMatchObject(expectedSchema);
        ...
        test(`getSchema(null, 'ru')`, () => {
          const val = instance.getSchema(null, 'ru');
                expect(val).toMatchObject(expectedSchemaRu);
                const values = instance._getValuesFromSchemaFragment(val).__root__.config1;
                expect(values).toMatchObject(expected);
        });

        test(`getSchema('${paramPath}')`, () => {
          ...
          const result = path ? values[path.split('.').pop()] : values.config1;
                expect(result).toMatchObject(expectedFragment);
        });

        test(`getSchema('${paramPath}', 'ru')`, () => {
