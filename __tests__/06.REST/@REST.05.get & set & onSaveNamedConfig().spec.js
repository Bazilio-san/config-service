...
const { cu, initWS, get, post, stopWS } = require('./init-test-web-server.js');
...
describe(`REST: get & set`, () => {
  ...
  describe(`ERRORS...`, () => {
        [
          ...
          test(`ERROR (${fn}): ${re.source}`, (done) => {
                post(`${cu}${fn}=${paramPath}`)
                    .send(value)
                    .expect(500)
                    .expect('Content-Type', /json/)
                    .end((err, res) => {
                        expect(res.body.err.message).toMatch(re);
                        done();
                    });
          });
        });
          ...
          test(`"${query}"`, (done) => {
                post(`${cu}${query}`)
                    ...
                    .end((err, res) => {
                        ...
                        expect(res.body).toMatchObject(true);
                        done();
                    });
          });

          test('Method onChange() should work', () => {
            const testValue = instance.testOnChange;
            const expected_ = paramPath + newVal;
                expect(testValue).equals(expected_);
          });

          test('Method onSaveNamedConfig() should work', () => {
  ...
  describe(`OnChange () method should not be called if noonchange parameter is passed`, () => {
    ...
    test(`Seting new value by "set=${paramPath}&noonchange" and checking that the onChange method was NOT called`, (done) => {
    ...
    test(`Checking a new value by "get":`, (done) => {
            get(`${cu}get=${paramPath}`)
                .end((err, res) => {
                    expect(res.body.value).toMatchObject(newVal1);
                    done();
                });
    });

    test(`Rolling back old value by call "set=${paramPath}" and checking that the onChange method was called`, (done) => {
    ...
    test(`Checking an old value by "get":`, (done) => {
            get(`${cu}get=${paramPath}`)
                .end((err, res) => {
                    expect(res.body.value).toMatchObject(oldVal);
                    done();
                });
    });
  });
  ...
  describe(`Checking a new values by "get":`, () => {
    each(expected12, (configExpected, configName) => {
      test(`Named config: "${configName}"`, (done) => {
                get(`${cu}get=${configName}`)
                    .end((err, res) => {
                        expect(res.body.value).toMatchObject(configExpected);
                        done();
                    });
      });
    });
        [...newValues, ...unchangedValues].forEach(([paramPath, paramValue]) => {
          test(`Path: "${paramPath}"`, (done) => {
                get(`${cu}get${paramPath === null ? '' : `=${paramPath}`}`)
                    .end((err, res) => {
                        expect(res.body.value).toMatchObject(paramValue);
                        done();
                    });
          });
        });
