...
const expectedSchema = cloneDeep(require('../resources/with-actual-values/schema---av.js'));
const expectedSchemaRu = cloneDeep(require('../resources/with-actual-values/schema-ru---av.js'));
const expectedSchemaEn = cloneDeep(require('../resources/with-actual-values/schema-en---av.js'));
...
describe(`REST: Method "get-schema" should work properly`, () => {
  beforeAll(initWS);
    ...
    [
      ...
      test(`"${cu}${query}" --> full schema`, (done) => {
            get(`${cu}${query}`)
    ...
    [
      ...
      test(`"${url}" --> full schema for ${name}`, (done) => {
            get(url)
                .expect(expected)
                .end(done);
      });
      ...
      test(`"${url}"`, (done) => {
        if (expected === 'error') {
                get(url)
                    .expect(500)
                    .expect((res) => {
                        expect(res.body.err.message).toMatch(match);
                    })
                    .end(done);
        } else {
                get(url)
