...
describe('Params: _readNamedConfig(), _saveNamedConfig(), _reloadConfig()', () => {
  describe('_readNamedConfig, _saveNamedConfig, _reloadConfig', () => {
    ...
    const expectedConfig = require('../00.new/expected-new-config.js');
        ...
        __.each(expectedConfig, (expectedValue, configName) => {
          test(`Check object "${configName}"`, () => {
            const config = instance._getValues();
                expect(config[configName]).toMatchObject(expectedValue);
          });
        ...
        test('When reloading a modified config, changes should be visible', () => {
          ...
          cpc('./config_02_2.json', configFile);
            ...
            const config = instance._getValues();
            expect(config.config_02.param_02).toMatchObject('version 2');
        });

        test('ERROR: Could not load named configuration file (1)', () => {
        ...
        test('ERROR: Could not load named configuration file (When creating a new service instance, named configurations should not be cached)', () => {
  ...
  describe('Overriding service directories should work properly', () => {
    ...
    test('Checking for presence of a Schema in an overridden directory', () => {
    ...
    test('Checking for named configuration in an overridden directory', () => {
      const config = instance._getValues();
